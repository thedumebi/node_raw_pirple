...
app.loadChecksListPage = function () {
  // Get the phone number from the current token, or log the usr out if none
  var phone =
    typeof app.config.sessionToken.phone === "string"
      ? app.config.sessionToken.phone
      : false;
      ...
      function (statusCode, responsePayload) {
        if (statusCode === 200) {
          ...
          if (allChecks.length > 0) {
            // show each check as a new row in the table
              ...
              app.client.request(
                ...
                function (statusCode, responsePayload) {
                  if (statusCode === 200) {
                    ...
                    // Make the check data into a tablel row
...
app.loadChecksEditPage = function () {
  // Get the check id from the query string, if nonw is found then redirct back to dashboard
  var id =
    ...
    app.client.request(
      ...
      function (statusCode, responsePayload) {
        if (statusCode === 200) {
          ...
          // Put the data into the form as values where needed
