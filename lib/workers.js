...
// when you want to debug you can then call NODE_DEBUG=workers node index.js
...
workers.validateCheckData = function (originalCheckData) {
  ...
  if (
    ...
    debug("Error: One of the checks is not formatted. Skipping it.");
  }
};
...
workers.log = function (
  ...
  // Determine the name of the log file we are writing to
  var logFileName = originalCheckData.id;

  // Append the log string to te file
    ...
    if (!err && logs && logs.length > 0) {
      logs.forEach(function (logName) {
        ...
        _logs.compress(logId, newFileId, function (err) {
          if (!err) {
            // Truncate the log (empty out the log)
